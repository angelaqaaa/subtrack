...
const EditSubscriptionModal = ({ show, onHide, onSuccess, subscription }) => {
  ...
  const loadUserSpaces = async () => {
    try {
      ...
      console.log('Edit Modal - Spaces API response:', response);
      if (response.status === 'success') {
        // Only show spaces where user has edit permissions
        const allSpaces = response.data?.spaces || [];
        console.log('Edit Modal - All spaces:', allSpaces);
        const editableSpaces = allSpaces.filter(space =>
          space.user_role === 'admin' || space.user_role === 'editor'
        );
        console.log('Edit Modal - Editable spaces:', editableSpaces);
        setSpaces(editableSpaces);
      } else {
        console.log('Edit Modal - Spaces API failed:', response);
      }
    } catch (err) {
      console.error('Edit Modal - Failed to load spaces:', err);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await subscriptionsAPI.update(subscription.id, formData);
      console.log('Update subscription response:', response);

      if (response.status === 'success') {
        // Log activity
        ActivityLogger.log(ActivityTypes.SUBSCRIPTION_UPDATED, {
